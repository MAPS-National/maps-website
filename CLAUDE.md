...
**Content bootstrap (local scratch → staging/prod).** Seeding a remote env from a laptop is unreliable (the tsx process exits before remote S3 uploads finish), so launch content goes up as a **bulk copy**, not a seed. Build + verify a clean set locally in a scratch DB/bucket (migrate → imports → `import:prose` → `import:slider` → `seed:pages`), then `npm run push:content -- --to staging|prod` (`scripts/push-content.mjs`, the launch counterpart of `refresh:staging`): mirrors the scratch MinIO bucket up and restores its `pg_dump` (full dump preserves ids). Source is locked to a **localhost** DB, the target is resolved from Railway by name, and `--to prod` **always** requires typing `push to prod` (`--yes` won't skip it). Defaults: `SRC_DATABASE_URL=…/payload_bootstrap`, `SRC_S3_BUCKET=bootstrap-media` (override via env). Verify a push by curling the target's Railway URL (not the custom domain — see below): every page + every `/api/media/file/*` it references should 200. A bulk copy carries the scratch's (empty) users, so the pushed env's `/admin` lands on create-first-user; `push:content` fixes this by running `ensure:admin` (`scripts/ensure-admin.ts`) as a final step when `ADMIN_EMAIL`/`ADMIN_PASSWORD` are set — idempotent (match by email, create or reset password), no S3 writes so it is safe against a remote DB. Run `npm run ensure:admin` standalone (with `DATABASE_URL` pointed at the target) to seed a login without a full push.

**Deploying that content: the `dev` migration-marker trap.** The scratch is built with dev `push:true`, which leaves a `dev` row in `payload_migrations`. A full dump carries it to the target, and the target's preDeploy `payload migrate` then detects dev mode and **prompts interactively** ("dev mode … data loss will occur, proceed? y/N") — which **hangs the non-TTY deploy container** (deploy never finishes). `push:content` now deletes that row after restore; if you hit a stuck prod deploy, `DELETE FROM payload_migrations WHERE name='dev'` then redeploy. Prod deploys are still **manual** `railway up --environment production --service web --ci` (no auto-deploy-on-push wired for prod).

**Custom domains are not cut over.** Prod runs on its Railway URL (`web-production-*.up.railway.app`); `mapsnational.org` still serves the **old Webflow** site (its responses carry `surrogate-key: pageId:` Webflow/Fastly headers). So "prod is broken" observed on `mapsnational.org` is Webflow, not this app — verify against the Railway URL. DNS cutover is the go-live step. Staging's `stage.mapsnational.org` **is** cut over (behind the Basic-auth gate).
