...
**Deploying that content: the `dev` migration-marker trap.** The scratch is built with dev `push:true`, which leaves a `dev` row in `payload_migrations`. A full dump carries it to the target, and the target's preDeploy `payload migrate` then detects dev mode and **prompts interactively** ("dev mode … data loss will occur, proceed? y/N") — which **hangs the non-TTY deploy container** (deploy never finishes). `push:content` and `refresh:staging` both delete that row after restore; if you still hit a stuck deploy (staging or prod), `DELETE FROM payload_migrations WHERE name='dev'` against that env's Postgres, then redeploy. This bit staging on its first auto-deploy: an earlier prod → staging refresh had carried the marker over before `refresh:staging` was patched to strip it.
