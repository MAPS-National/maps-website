...
- **M1 done** — the 2 supplied community photos (Georgetown Law panel + MAPS at the U.S. DOJ) are in the compact grid (now 9). Converted to optimized webp (the DOJ one from HEIC via a transient decoder), committed under `public/import/prose/`. `seed-pages` `mediaId` is now self-healing — it creates a Media doc from the tracked source if absent, so a fresh DB reproduces the gallery without `import:prose`.
