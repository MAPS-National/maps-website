...
- [ ] **G9 — Members nav column renders empty for logged-out visitors** → **CONFIRMED real (via screenshot).** React renders all 7 Members links, but **Outseta's no-code module injects `<style id="o-hide-content-groups">` that hides every `/members/*` link** (a protected content group) for anonymous users — leaving the "Members" heading with nothing under it. (Proof: a freshly created `<a href="/members/…">` also computes `display:none`; a `/programs` one stays visible.) It only looks correct when signed in. Decide the anonymous-user UX: hide the whole Members group when its links are hidden, OR show a "Member login" prompt under it, OR exclude the nav links from Outseta's content-group hiding. _(HITL — anonymous-user UX)_
