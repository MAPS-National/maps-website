...
- **Merge method is rail-dependent. Feature to `staging` PRs squash-merge** (tidy history, one commit per change). **The `staging` to `master` promotion PR must be a real merge commit, never a squash.** A squash promotion mints a fresh `master` commit that shares no ancestry with `staging`'s commits, so the two branches — though tree-identical right after — diverge in history. Every later `master...staging` compare (and each promotion PR) then re-reports everything since the merge-base as "new", dominated by the two ~19k-line Drizzle schema snapshots, and needs a manual "merge master into staging to reconcile" commit to clear. A merge-commit promotion keeps `master`'s ancestry continuous with `staging`, so the merge-base stays current and the phantom diff never appears. (Both methods are enabled repo-wide; this is a per-PR choice the promoter must make — pick "Create a merge commit" on the promotion PR.)
