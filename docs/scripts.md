...
| `npm run reindex:search` | `reindex-search.ts` | env (via `DATABASE_URL`) | Rebuild the search index for every published page + post by re-running the search plugin's afterChange sync (no-op re-publish), so search reflects the **current** code's indexing. In-process (uses this repo's code); only point at an env that runs this same code, else use the deployed app's Reindex button. Touches no S3. Idempotent; best-effort per doc (a doc that fails validation keeps its old entry). Run after `refresh:local` (which now calls it) or a `beforeSync` change. |
